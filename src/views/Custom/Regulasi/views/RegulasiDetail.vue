...
@Component({
  components: { Panel, Editor, UploadInput, Attachments }
})
export default class KepesertaanAdd extends Vue {
  ...
  async fetchData() {
    try {
      ...
      const { data } = await axios.get(
        'regulasi/get/detail/' + this.selected_id
      );
  ...
  async submit() {
    try {
      ...
      await axios.put('regulasi/update/' + this.selected_id, this.dataToPost);
      this.loading = false;
    } catch (error) {
      this.loading = false;
    }
  ...
  async remove() {
    this.$swal({
      ...
      if (result.isConfirmed) {
        try {
          this.loading = true;
          await axios.delete('regulasi/delete/' + this.selected_id);
          this.$router.push('/regulasi');
        } catch (error) {
          this.loading = false;
        }
      }
